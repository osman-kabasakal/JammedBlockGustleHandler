...
export default class GamesScreen extends MainLayout {
    ...
    renderOyun(oyunlar:OyunData[],id:string):JSX.Element|JSX.Element[]{
        ...
        for(let oyun of oyunlar){
            oyunlarElem.push(( <Card key={id+oyunlar.indexOf(oyun)}
                containerStyle={{
                  margin: 2,
                  width:"45%"
                }}>
                    <TouchableOpacity
          onPress={(event)=>{this.props.navigation.navigate("Game",{id:id,index:oyunlar.indexOf(oyun)})}}
          >
  <Image
  source={ oyun.screen}
  style={{ height:100 }}
